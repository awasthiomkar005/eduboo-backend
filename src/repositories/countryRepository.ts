...
export class CountryRepository {
 ...
 async findAll(page: number = 1, limit: number = 10) {
  ...
  async getCategoriesCoursesPricingByCountryNamePaginated(countryName: string, skip: number = 0, take: number = 10) {
    ...
    console.log("Searching for country:", trimmedCountryName);

    // First, verify the country exists
    const country = await this.db.country.findFirst({
      where: { country_name: { equals: trimmedCountryName, mode: 'insensitive' } },
    });

    console.log("Country found:", country);

    if (!country) {
      throw new Error(`Country '${trimmedCountryName}' not found`);
    }
    ...
    const courseIds = groups.map((group: any) => group.course_id);
    ...
    // ... existing code ...
    const pricings = await this.db.coursePricing.findMany({
      where: {
        country_id: country.country_id,
    ...
    pricings.forEach((pricing) => {
      if (!pricingMap.has(pricing.course_id)) {
        pricingMap.set(pricing.course_id, pricing);
      }
    });

    // Enrich courses with pricing information and module count
    const coursesWithPricing = courses.map((course) => ({
      ...course,
      number_of_modules: course.modules.length,
      pricing: pricingMap.get(course.course_id) || null,
    }));

    courses.forEach((course: any) => { delete course.modules; });

    const orderMap = new Map(courseIds.map((id, idx) => [id, idx] as const));
    return coursesWithPricing.sort((a: any, b: any) => (orderMap.get(a.course_id)! - orderMap.get(b.course_id)!));
  }

  async countDistinctCoursesByCountryName(countryName: string): Promise<number> {
    // First verify the country exists
    const country = await this.db.country.findFirst({
      where: { country_name: { equals: countryName, mode: 'insensitive' } },
    });
