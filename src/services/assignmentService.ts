...
export class AssignmentService {
  ...
  async createAssignment(assignmentData: CreateAssignmentData) {
    ...
    // Fan out StudentAssignment rows for all active enrollments in the course
    try {
      await this.assignmentRepository.createStudentAssignmentsForCourse(
        assignment.course_id,
        assignment.assignment_id
      );
    } catch (err) {
      console.error('Failed to create StudentAssignment records:', err);
    }
