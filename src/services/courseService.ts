...
export interface CreateCourseData {
  ...
  min_age?: string;
  max_age?: string;
  course_type?: 'one_to_one' | 'one_to_many' | 'recorded';
}
...
export class CourseService {
  ...
  // }
async getAllCourses(page: number, limit: number, courseType?: 'one_to_one' | 'one_to_many' | 'recorded') {
  return await this.courseRepository.findAll(page, limit, courseType);
}
  ...
  async getCourseById(courseId: string) {
    ...
    const countries = await this.getCountries();
    const pricingByCountry = await this.courseRepository.getPricingForCourseAcrossCountries(course.course_id);
    // console.log("pricingByCountry raw data:", JSON.stringify(pricingByCountry, null, 2));
    const pricingMap = new Map(pricingByCountry.map((p: any) => [p.country_id, p]));
  ...
  async getModulesWithChapters(courseId: string) {
    ...
    return this.courseRepository.getModulesWithChapters(courseId);
  }

  async getTopSellingCourses(countryName: string, limit: number = 5) {
    const courses = await this.courseRepository.getTopSellingCoursesByCountry(countryName, limit);
    return courses;
  }

  async getLowestSellingCourses(countryName: string, limit: number = 5) {
    const courses = await this.courseRepository.getLowestSellingCoursesByCountry(countryName, limit);
    return courses;
  }
}
