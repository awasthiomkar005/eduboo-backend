...
export class UserService {
  ...
  async getLeaderboard(userId: string) {
    if (!userId) {
      throw new CustomError('user_id is required', HTTP_STATUS.BAD_REQUEST);
    }

    const [topThree, user] = await Promise.all([
      this.userRepository.getTopByXp(3),
      this.userRepository.getUserXp(userId),
    ]);
    ...
    // Optionally compute rank of the user
    const higherCount = await this.userRepository.countUsersWithXpGreaterThan(user.xp || 0);
