...
export class SessionController {
 ...
 getUpcomingClassesByStudent = asyncHandler(async (req: Request, res: Response) => {
  const studentId = req.params.studentId;
  const { page = '1', limit = '10', dateFrom, dateTo, courseId } = req.query;
  ...
  const result = await sessionService.getUpcomingClassesByStudent(studentId, filters);
